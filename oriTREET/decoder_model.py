...
from .transformerDecoder import Decoder, DecoderLayer
from .attention import FullFixedTimeCausalConstructiveAttention, AttentionLayer
from .embed import DataEmbedding_wo_temp
...
class Model(nn.Module):
    ...
    def __init__(self, configs):
        ...
        assert (
            configs["label_len"] >= 0 and configs["pred_len"] > 0
        ), "Label length is non negative and pred length is positive"
        if configs["label_len"] > 0:
            print(
                f"Label length is greater than 0 - no gradients will be taken for the first {configs['label_len']} steps."
            )
        ...
        # Basic:
        self.y_dim = (
            configs["y_dim"] + self.x_in * configs["x_dim"]
        )  # inserting x with y
        ...
        self.dec_embedding = DataEmbedding_wo_temp(
            self.y_dim, self.emb_out, configs["dropout"]
        )
    ...
    def forward(self, y, y_tilde, x=None):
        assert (
            x is not None if self.x_in else True
        ), "x_in is true but no batch x found."
        ...
        dec_out = self.decoder(dec_out, x_mask=None, drawn_y=False)

        dec_outs_tilde = []
        for i in range(self.n_draws):
            y_tilde_inp = self.draw_y(y, y_tilde)
            y_tilde_inp = (
                torch.cat((y_tilde_inp, x), dim=-1) if self.x_in else y_tilde_inp
            )

            dec_out_tilde = self.dec_embedding(y_tilde_inp)
            # dec_out_tilde, attns_tilde = self.decoder(
            #     dec_out_tilde, x_mask=None, drawn_y=True
            # )
            dec_out_tilde = self.decoder(dec_out_tilde, x_mask=None, drawn_y=True)
