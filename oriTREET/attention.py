...
class FullFixedTimeCausalConstructiveAttention(nn.Module):
    ...
    def forward(self, queries, keys, values, attn_mask, drawn_y=False):
        ...
        if self.history_len is not None and drawn_y:
            future_scores_with_drawn_keys = torch.einsum(
                "blhe,blhe->bhl",
                queries[:, self.history_len :],
                keys_drawn[:, self.history_len :],
            )
            ...
            partial_eye_mask = torch.diag(
                torch.cat(
                    [torch.zeros(self.history_len), torch.ones(L - self.history_len)]
                )
            ).to(scores.device)[:, :S]
            # the partial eye mask is for the drawn values
            scores = (
                scores_with_drawn_keys * partial_eye_mask
                + (1 - partial_eye_mask) * scores
            )

        if self.mask_flag:
            if attn_mask is None:
                # attn_mask = TriangularCausalMask(B, L, device=queries.device)
                attn_mask = DoubleTriangularCausalMask(
                    B, L, diagonal=1, history=self.history_len, device=queries.device
                )
        ...
        if self.history_len is not None and drawn_y:
            ...
            values_drawn = (
                values_drawn.permute(0, 2, 3, 1).unsqueeze(-2).repeat(1, 1, 1, L, 1)
            )
            # the partial eye mask is for the drawn values
